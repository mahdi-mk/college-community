...
<div class="d-flex gap-1">
  <button class="action-button action-button-primary" :class="{'active': hasUpvote}" @click="toggleUpvote">
  ...
  <button class="action-button action-button-danger" :class="{'active': hasDownvote}" @click="toggleDownvote">
...
export default {
  ...
  data() {
    return {
      hasUpvote: this.answer.auth_user_vote?.type == 1,
      hasDownvote: this.answer.auth_user_vote?.type == 0
    }
  },
  methods: {
    toggleUpvote() {      
      this.$http.post(`/answers/${this.answer.id}/upvote/toggle`).then((response) => {
    ...
    toggleDownvote() {
        this.$http.post(`/answers/${this.answer.id}/downvote/toggle`).then((response) => {
